...
### JSON shape (parse carefully)

The top-level JSON type differs by command — mismatching it throws `'list' object has no attribute 'get'`:

| Command | `--json` returns |
|---------|------------------|
| `br list` / `br ready` / `br blocked` | **object**: `{"issues": [...], "total": N, ...}` → read `.issues` / `.total` |
| `br update` / `br show` / `br close` / `br create` | **array** `[ {...} ]` (these accept multiple ids) → take `[0]` |

Normalize before reading fields: `d = d[0] if isinstance(d, list) else d.get("issues", [d])[0]`.
Better for mutations: don't parse `update`/`claim`/`close` output at all — you already know the id you passed; just check the exit code.
